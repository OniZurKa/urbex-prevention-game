...
static Rectangle settings_button = {0};
...
void menu_init_layout(int window_width, int window_height) {
    ...
    settings_button = (Rectangle){center_x - 150.0f, base_y + 74.0f, 300.0f, 56.0f};
    quit_button = (Rectangle){center_x - 150.0f, base_y + 148.0f, 300.0f, 56.0f};
}

void menu_update(game_state_t *state, bool *is_running) {
    Vector2 mouse = GetMousePosition();
    bool hover_start = CheckCollisionPointRec(mouse, start_button);
    bool hover_settings = CheckCollisionPointRec(mouse, settings_button);
    ...
    if (hover_settings && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
        *state = GAME_STATE_SETTINGS;
        return;
    }

    if (IsKeyPressed(KEY_ESCAPE) || (hover_quit && IsMouseButtonPressed(MOUSE_LEFT_BUTTON))) {
...
void menu_draw(int window_width, int window_height) {
    ...
    draw_button(settings_button, "PARAMETRES", hover_settings, (Color){100, 100, 100, 255});
